...
from pysat.card import CardEnc, EncType, IDPool
...
def sequential_counter_amo(cnf: CNF, lits: List[int], vpool: IDPool):
    n = len(lits)
    if n <= 1:
        return

    # Create auxiliary variables s1, s2, ..., s(n-1)
    aux_vars = [vpool.id() for _ in range(n - 1)]

    # x1 -> s1  <==>  ¬x1 ∨ s1
    cnf.append([-lits[0], aux_vars[0]])

    for i in range(1, n - 1):
        # xi -> si  <==>  ¬xi ∨ si
        cnf.append([-lits[i], aux_vars[i]])

        # s(i-1) -> si  <==>  ¬s(i-1) ∨ si
        cnf.append([-aux_vars[i - 1], aux_vars[i]])

        # ¬(xi ∧ s(i-1))  <==>  ¬xi ∨ ¬s(i-1)
        cnf.append([-lits[i], -aux_vars[i - 1]])

    # Last variable: ¬(xn ∧ s(n-2))
    cnf.append([-lits[n - 1], -aux_vars[n - 2]])
...
def solve_single_instance(instance_path: str, log_dir: str, solver_cls) -> dict:
    """Solve a single instance & return results as a dict"""
    ...
    with Manager() as manager:
        shared = manager.dict()
        p = Process(target=solve_inc, args=(n, edges, k, UB, shared, solver_cls))
        ...
        # Write log file (không tính vào thời gian)
...
def build_base_cnf(n: int, edges: List[Tuple[int, int]], k: int, UB: int) -> CNF:
    ...
    # K[i,j] <-> X[i,j] & ¬X[i,j-1]
    for i in range(n):
        cnf.append([-K_id(i, 1, n, UB), X_id(i, 1, UB)])
        cnf.append([K_id(i, 1, n, UB), -X_id(i, 1, UB)])
    ...
    # 4 khong có 2 đỉnh cùng nhãn
    # pair wise
    # for i1 in range(n):
    #     for i2 in range(i1 + 1, n):
    #         for j in range(1, UB + 1):
    #             cnf.append([-K_id(i1, j, n, UB), -K_id(i2, j, n, UB)])
    # sử dụng sequential_counter_amo
    # for j in range(1, UB + 1):
    #     k_vars_for_label_j = [K_id(i, j, n, UB) for i in range(n)]
    #     sequential_counter_amo(cnf, k_vars_for_label_j, vpool)

    # sử dụng thư viện enctype
    for j in range(1, UB + 1):
        k_vars_for_label_j = [K_id(i, j, n, UB) for i in range(n)]

        clauses = CardEnc.atmost(lits=k_vars_for_label_j, bound=1, vpool=vpool, encoding=1)
    ...
    for u, v in directed_edges:
        for val in range(1, UB + 1):
            ...
            if val - k >= 1:
                cond.append(X_id(v, val - k, UB))
            if val + k - 1 <= UB:
                cond.append(-X_id(v, val + k - 1, UB))

            if cond:
                cnf.append([-K_id(u, val, n, UB)] + cond)
            else:
...
def solve_inc(n, edges, k, UB, shared, solver_cls=Glucose42):
    cnf = build_base_cnf(n, edges, k, UB)
    solver = solver_cls(bootstrap_with=cnf.clauses)

    print("Vars:", solver.nof_vars())
    ...
    while high >= n:
        ...
        # sb_assumption = X_id(sb_vertex, (high+1)//2, UB)
        # sat = solver.solve(assumptions=[-C, sb_assumption])

        # assumption khi không dùng biến phụ

        # assumptions = [X_id(i, high, UB) for i in range(n)]
        # sb_assumption = X_id(sb_vertex, (high + 1) // 2, UB)

        if high != UB:
            for i in range(n):
                solver.add_clause([X_id(i, high, UB)])
    ...
    for i in range(n):
        print(f"Vertex {i} -> label {labels[i]}")


def main():
    parser = argparse.ArgumentParser(
        description='L(k,1)-Labeling SAT Solver - Batch Processing',
    ...
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['instance', 'n', 'm', 'k', 'UB', 'span', 'time',
                      'variables', 'clauses', 'solver', 'status']
